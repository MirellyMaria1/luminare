
import 'package:untitled3/domain/cardfavs.dart';

class Database2 {
  static List cardfav = [
    Cardfavorito(
      urlImage:
          'https://images.tcdn.com.br/img/img_prod/848976/anel_solitario_em_ouro_18k_com_cristal_azul_safira_1829_1_07fe536714dbff6bc25aaafe579b4068.jpeg',
      titulo: 'anel',
      descricao: 'banhados a ouro> SEMIJOIAS > BRINCOS > kits de brincos',
      valor: 'R\$ 199,00',
    ),
    Cardfavorito(
      urlImage:
          'https://down-br.img.susercontent.com/file/c84e22a0ae7163f33fa350942648b024',
      titulo: 'Kit de colares trançados',
      descricao: 'banhados a ouro> SEMIJOIAS > COLARES > kits de colares',
      valor: 'R\$ 199,00',
    ),
    Cardfavorito(
      urlImage:
          'https://images.tcdn.com.br/img/img_prod/848976/anel_solitario_em_ouro_18k_com_cristal_azul_safira_1829_1_07fe536714dbff6bc25aaafe579b4068.jpeg',
      titulo: 'anel',
      descricao: 'banhados a ouro> SEMIJOIAS > ANÉIS > anéis cravejados',
      valor: 'R\$ 199,00',
    ),
    Cardfavorito(
      urlImage:
          'https://www.bwimg.com.br/maisejoias/produtos/pulseira-de-missangas-coloridas-folheada-a-ouro-18k-1637260750.4232.jpg',
      titulo: 'bracelete cristal multicolor',
      descricao: 'SEMIJOIAS > PULSEIRA > BRACELETE>pedras cravejadas',
      valor: 'R\$ 199,00',
    ),
    Cardfavorito(
      urlImage:
          'https://images.tcdn.com.br/img/img_prod/848976/anel_solitario_em_ouro_18k_com_cristal_azul_safira_1829_1_07fe536714dbff6bc25aaafe579b4068.jpeg',
      titulo: 'anel',
      descricao: 'banhados a ouro> SEMIJOIAS > BRINCOS > kits de brincos',
      valor: 'R\$ 199,00',
    ),
    Cardfavorito(
      urlImage:
          'https://down-br.img.susercontent.com/file/c84e22a0ae7163f33fa350942648b024',
      titulo: 'Kit de colares trançados',
      descricao: 'banhados a ouro> SEMIJOIAS > COLARES > kits de colares',
      valor: 'R\$ 199,00',
    ),
    Cardfavorito(
      urlImage:
          'https://images.tcdn.com.br/img/img_prod/848976/anel_solitario_em_ouro_18k_com_cristal_azul_safira_1829_1_07fe536714dbff6bc25aaafe579b4068.jpeg',
      titulo: 'anel',
      descricao: 'banhados a ouro> SEMIJOIAS > ANÉIS > anéis cravejados',
      valor: 'R\$ 199,00',
    ),
    Cardfavorito(
      urlImage:
          'https://www.bwimg.com.br/maisejoias/produtos/pulseira-de-missangas-coloridas-folheada-a-ouro-18k-1637260750.4232.jpg',
      titulo: 'bracelete cristal multicolor',
      descricao: 'SEMIJOIAS > PULSEIRA > BRACELETE>pedras cravejadas',
      valor: 'R\$ 199,00',
    ),
  ];

  rawQuery(String sql) {}
}
